...
const Message = ({ id, content, isUser, timestamp, botLogoUrl, isError, audioEnabled }) => {
  ...
  const handleEvaluation = async (messageId, messageContent, rating) => {
    ...
    try {
      const response = await fetch('http://localhost:8000/message/evaluate', {
      method: 'POST',
      headers: {
  ...
  try {
    ...
    const generateUrl = `http://localhost:8000/message/voice/generate?message_id=${messageId}&response_sent=${encodedResponse}`;
    const generateResponse = await fetch(generateUrl, {
      method: 'POST'
    });

    if (generateResponse.ok) {
      const getUrl = `http://localhost:8000/message/voice/get_url?message_id=${messageId}`;
...
function updateLocalMessages(newMessage, role) {
  ...
  const fetchMessages = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`http://localhost:8000/chats/${current_chat_id}/messages`);
      if (!response.ok) {
  ...
  useEffect(() => {
    const fetchLabCodingLanguage = async () => {
      try {
        const response = await fetch(`http://localhost:8000/labs/${lab_id}/details`);
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchChats = async () => {
      try {
        const response = await fetch(`http://localhost:8000/chats/active/${user_id}/details`);
  ...
  const startNewChatSubmit = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8000/chats/add', {
        method: 'POST',
...
const handleSubmit = async (event) => {
  if ((event.key === 'Enter' && !event.shiftKey) || event.type === 'submit') {
    ...
    updateLocalMessages(messageData, "user");
    setQuery("");
    const textarea = document.getElementById('resizable-textarea');
    if (textarea) {
    ...
    try {
      await generateBotResponse(messageData); // This now also updates the bot's messages locally
      await fetchMessages(current_chat_id); // Fetches all messages including the new bot's message
    } catch (error) {
      console.error('Error during message handling:', error);
    } finally {
      setIsStreaming(false);
      await fetchChats(); // Optionally refresh chats if needed for UI updates
    }
  }
};
...
const generateBotResponse = async (messageData) => {
  ...
  try {
    const response = await fetch(`http://localhost:8000/message/generate/async`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    while (true) {
      ...
      // Update the latest message part to the UI
      setMessages(messages => {
        const updatedMessages = [...messages];
        const lastMessageIndex = updatedMessages.length - 1;
        updatedMessages[lastMessageIndex].body = accumulatedMessage;
    ...
    if (error.name === 'AbortError') {
      ...
      throw new Error(`Backend API error: ${error.statusText}`);
    }
  } finally {
    setIsStreaming(false); // Ensure streaming state is reset
    setAbortController(null); // Clear the controller from the state
  }
};

const handleStopGeneration = () => {
  if (abortController) {
    abortController.abort(); // Use the controller from the state to abort the request

    setIsStreaming(false);
    notification.info({
      message: 'Stopping...',
      description: 'The message generation is being stopped. Please wait...',
      duration: 1.5
    });
    ...
    setTimeout(() => {
      notification.success({
        message: 'Stopped',
        description: 'Message generation has been successfully stopped.',
        duration: 2
      });
    }, 1500);
  }
...
const fetchLabVoiceDetail = async (lab_id) => {
  const response = await fetch(`http://localhost:8000/labs/${lab_id}/details/voice_response`)
  if (!response.ok)
  {
...
useEffect(() => {
  ...
  const simulateBotResponseToRecommendation = async (recommendation) => {
    const new_message = {
      ...
      updateLocalMessages(new_message, "user");
